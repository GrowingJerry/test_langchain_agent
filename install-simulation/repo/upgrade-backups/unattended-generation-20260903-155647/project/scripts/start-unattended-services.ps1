...
if (-not $listener) {
    ...
    Set-Content -LiteralPath (Join-Path $logDir "streamlit.pid") -Value $streamlit.Id -Encoding ASCII
    Write-Host "[OK] Streamlit started. PID=$($streamlit.Id)"
} else { Write-Host "[OK] Streamlit is already listening on port $Port." }

$supervisor = Start-Process -FilePath "powershell.exe" -ArgumentList @("-NoProfile","-ExecutionPolicy","Bypass","-File",(Join-Path $PSScriptRoot "run-generation-worker-supervisor.ps1"),"-Python",$Python,"-Database",$Database) -WorkingDirectory $projectRoot -WindowStyle Hidden -RedirectStandardOutput (Join-Path $logDir "supervisor.out.log") -RedirectStandardError (Join-Path $logDir "supervisor.err.log") -PassThru
Set-Content -LiteralPath (Join-Path $logDir "supervisor.pid") -Value $supervisor.Id -Encoding ASCII
