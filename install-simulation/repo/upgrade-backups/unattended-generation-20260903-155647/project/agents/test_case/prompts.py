...
18. 当前调用只处理 GenerationPackage 指定的原子需求/测试点批次；完整需求由程序分批、
    核对和合并。必须返回本批 cases，并原样保留 atomic_requirement_id 与 test_point_id。
19. 每条用例必须提供 structured_steps；每步只有一个动作，控件名用〖〗包裹，写明
    页面和区域，输入/选择给出具体值，预期只描述该步后立即可观察的结果。
20. element_id 和 page_id 只能来自 GenerationPackage。没有已确认页面证据时使用纯需求
    模式并标记待确认，禁止编造页面、控件、位置或保存成功。
21. 禁止“正常显示”“正确处理”等主观表述；服务端和数据库结果必须写入
    online_confirmation 并设置 need_human_confirm=true。
"""
