...
def test_long_running_batch_is_not_limited_by_legacy_threshold():
    guard=StreamGuard(Settings(generation_max_seconds=10, generation_model_call_max_seconds=0,
                               generation_batch_max_seconds=0))
    assert guard.reason("{}", elapsed=9, idle_elapsed=0) == ""

def test_idle_timeout_is_distinct_from_model_call_timeout():
    guard=StreamGuard(Settings(generation_model_call_max_seconds=1000,
                               generation_idle_timeout_seconds=10))
    assert guard.reason("{}", elapsed=500, idle_elapsed=11) == "idle_timeout"
    assert guard.reason("{}", elapsed=1001, idle_elapsed=0) == "model_call_timeout"

def test_stream_activity_callback_receives_each_heartbeat():
    activity=[]
    events=[{"message":{"thinking":"working","content":""}},
            {"message":{"content":"{\"ok\":true}"}}]
    _, content=StreamGuard(Settings()).collect(events, activity_callback=activity.append)
    assert content == '{"ok":true}' and len(activity) == 2
