...
from infrastructure.llm.stream_guard import GenerationTerminated
...
class UIApplicationService:
    ...
    def __init__(
        ...
        self.generation_jobs = GenerationJobService(manager)
    ...
    def enqueue_generation_job(self, request: GenerationRequest, requirement_ids: List[str],
                               batch_id: str, *, force: bool = False) -> Dict[str, Any]:
        job = self.generation_jobs.enqueue(request.project_id, batch_id,
            request.model_dump(mode="json"), requirement_ids,
            self.settings.model_dump(mode="json"), force=force)
        self._ensure_generation_worker()
        return job

    def generation_job(self, job_id: str) -> Dict[str, Any] | None:
        return self.generation_jobs.get(job_id)

    def generation_job_history(self, project_id: str) -> List[Dict[str, Any]]:
        return self.generation_jobs.latest(project_id)

    def cancel_generation_job(self, project_id: str, job_id: str) -> bool:
        return self.generation_jobs.request_cancel(project_id, job_id)

    def _ensure_generation_worker(self) -> None:
        script = Path(__file__).resolve().parents[2] / "scripts" / "run_generation_worker.py"
        db_path = self.manager.connections.db_path
        log_dir = Path(__file__).resolve().parents[2] / "logs" / "generation-worker"
        log_dir.mkdir(parents=True, exist_ok=True)
        kwargs: Dict[str, Any] = {"cwd": str(script.parent.parent)}
        if os.name == "nt":
            kwargs["creationflags"] = subprocess.CREATE_NEW_PROCESS_GROUP | subprocess.DETACHED_PROCESS
        else:
            kwargs["start_new_session"] = True
        with (log_dir / "worker.out.log").open("a", encoding="utf-8") as output:
            subprocess.Popen([sys.executable, "-X", "faulthandler", str(script), "--db", str(db_path),
                "--idle-exit-seconds", str(self.settings.generation_worker_idle_exit_seconds)],
    ...
    def generate(
        ...
        if request.generation_strategy != "legacy" and request.requirement_ids and not request.scenario_ids:
            return self.generation.generate_atomic_coverage(
    ...
    def generate_requirement_batch(
        ...
        for index, requirement_id in enumerate(ordered_ids, 1):
            batch_limit = self.__dict__.get("settings", default_settings).generation_batch_max_seconds
            if batch_limit and time.monotonic() - batch_started > batch_limit:
                summary["failed"].append({"requirement_id": requirement_id,
                    "error": "GenerationTerminated: batch_max_runtime_exceeded"})
                summary["termination_reason"] = "batch_max_runtime_exceeded"
                break
            ...
            try:
                ...
                coverage_complete = result.coverage_status in {"", "completed"}
                was_cancelled = result.coverage_status == "cancelled"
                requirement_status = "completed" if coverage_complete else ("cancelled" if was_cancelled else "needs_review")
                summary["requirements"].append({
                    "requirement_id":requirement_id, "status":requirement_status,
                    "generated":len(result.cases), "valid":result.valid_case_count,
                    "review_required":result.review_case_count,
                    "hard_rejected":result.rejected_case_count,
                    "failure_reason":"" if coverage_complete else f"missing test points: {result.missing_test_point_ids}",
                })
                ...
                if coverage_complete:
                    ...
                    summary["cancelled"] = True
                    summary["cancelled_requirement_id"] = requirement_id
                    summary["missing_test_point_ids"] = result.missing_test_point_ids
                    relay({"kind": "batch", "content": f"{requirement_id} 已停止；已完成批次保留。"})
                else:
                    summary["needs_review"].append({"requirement_id": requirement_id,
                        "missing_test_point_ids": result.missing_test_point_ids})
                    relay({"kind": "error", "content": f"{requirement_id} 覆盖不完整，需审核：{result.missing_test_point_ids}"})
        ...
        for index,node in enumerate(nodes,1):
            ...
            try:
                ...
                if exc.reason == "client_cancelled":
                    summary["cancelled"] = True
                    summary["cancelled_requirement_id"] = requirement_id
                    relay({"kind": "batch", "content": f"{requirement_id} 已由明确停止信号取消；已完成批次保留。"})
                    break
                summary["failed"].append({"requirement_id": requirement_id,
                    "error": f"GenerationTerminated: {exc.reason}"})
            except Exception as exc:
