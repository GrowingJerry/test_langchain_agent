...
SETTINGS=Settings(enable_ollama=True,enable_agent=True,ollama_timeout=300,ollama_num_ctx=8192,ollama_structured_num_predict=4096,
    text_model='qwen3:8b',vision_model='qwen2.5vl:3b',requirement_atomizer_model='qwen3:8b',
    requirement_auditor_model='qwen3:8b',page_understanding_model='qwen2.5vl:3b',
    test_case_model='qwen3:8b',test_case_review_model='qwen3:8b',
    generation_min_cases_per_atom=4,generation_default_max_cases_per_atom=6,
    generation_absolute_max_cases_per_atom=10,generation_max_cases_per_model_call=8,
    generation_auto_batch_enabled=True,generation_coverage_repair_enabled=True,
    generation_coverage_repair_max_rounds=2,generation_hard_max_cases_per_requirement=500)
...
def one_flow(manager,service,name,doc,expected_method):
    ...
    req=GenerationRequest(project_id=project_id,requirement_ids=ids,case_type='功能测试',case_count=6,
        max_cases_per_atom=6,auto_case_count=True,requested_mode='auto',use_project_kb=True,use_history=False)
    ...
    exported_rows=service.export_rows(project_id); coverage=exported_rows['atomic_coverage_matrix']
    covered={x['indicator_id'] for x in coverage if x.get('coverage_status') in {'covered','confirmed','proposed'}}
    required={x['indicator_id'] for x in atom_rows}; assert required<=covered
    test_point_rows=exported_rows['test_point_coverage_matrix']; assert test_point_rows
    ...
    with manager.connections.connection() as conn:
        point_count=conn.execute('SELECT COUNT(*) FROM coverage_test_points WHERE project_id=?',(project_id,)).fetchone()[0]
        batch_count=conn.execute('SELECT COUNT(DISTINCT generation_batch_id) FROM case_test_point_links WHERE project_id=?',(project_id,)).fetchone()[0]
        missing_count=conn.execute("SELECT COUNT(*) FROM coverage_test_points WHERE project_id=? AND status!='generated'",(project_id,)).fetchone()[0]
    return {'project_id':project_id,'method':parsed['extraction_method'],'records':records,'atoms':len(atom_rows),
        'test_points':point_count,'batches':batch_count,'missing':missing_count,'final_status':'completed' if not missing_count else 'needs_review',
        'pages':len(site['pages']),'source_bytes':sum(x.get('source_bytes',0) for x in site['pages']),
        'compressed_chars':sum(x.get('compressed_chars',0) for x in site['pages']),
        'binding':{k:binding[k] for k in ('participating','confirmed','need_human_confirm','unmatched','failed')},
        'cases':len(generated['cases']),'coverage_rate':len(covered)/len(required),'export':str(export),'chromium':health.get('chromium_version')}
