...
def migrate_database(connections: SQLiteConnectionManager) -> None:
    ...
    with connections.transaction() as conn:
        ...
        conn.executescript(
            """
            CREATE TABLE IF NOT EXISTS coverage_test_points (
                project_id TEXT NOT NULL, requirement_id TEXT NOT NULL,
                atomic_requirement_id TEXT NOT NULL, test_point_id TEXT NOT NULL,
                title TEXT NOT NULL, scenario_type TEXT NOT NULL, description TEXT NOT NULL,
                priority TEXT NOT NULL DEFAULT 'P1', evidence_requirement TEXT NOT NULL DEFAULT '',
                status TEXT NOT NULL DEFAULT 'pending', generation_batch_id TEXT NOT NULL DEFAULT '',
                metadata_json TEXT NOT NULL DEFAULT '{}', created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
                updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
                PRIMARY KEY(project_id,test_point_id)
            );
            CREATE TABLE IF NOT EXISTS atomic_generation_checkpoints (
                project_id TEXT NOT NULL, requirement_id TEXT NOT NULL,
                atomic_requirement_id TEXT NOT NULL, request_fingerprint TEXT NOT NULL,
                status TEXT NOT NULL, planned_count INTEGER NOT NULL DEFAULT 0,
                generated_count INTEGER NOT NULL DEFAULT 0, missing_test_point_ids_json TEXT NOT NULL DEFAULT '[]',
                repair_round INTEGER NOT NULL DEFAULT 0, last_error TEXT NOT NULL DEFAULT '',
                updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
                PRIMARY KEY(project_id,requirement_id,atomic_requirement_id,request_fingerprint)
            );
            CREATE TABLE IF NOT EXISTS case_test_point_links (
                project_id TEXT NOT NULL, case_id TEXT NOT NULL, test_point_id TEXT NOT NULL,
                atomic_requirement_id TEXT NOT NULL, generation_batch_id TEXT NOT NULL DEFAULT '',
                data_variant TEXT NOT NULL DEFAULT '', scenario_variant TEXT NOT NULL DEFAULT '',
                PRIMARY KEY(project_id,case_id,test_point_id)
            );
            CREATE TABLE IF NOT EXISTS generation_jobs (
                job_id TEXT PRIMARY KEY, project_id TEXT NOT NULL, batch_id TEXT NOT NULL,
                status TEXT NOT NULL, request_json TEXT NOT NULL, requirement_ids_json TEXT NOT NULL,
                settings_json TEXT NOT NULL, result_json TEXT NOT NULL DEFAULT '{}',
                force_regenerate INTEGER NOT NULL DEFAULT 0, progress_current INTEGER NOT NULL DEFAULT 0,
                progress_total INTEGER NOT NULL DEFAULT 0, stage TEXT NOT NULL DEFAULT 'queued',
                last_message TEXT NOT NULL DEFAULT '', last_error TEXT NOT NULL DEFAULT '',
                attempt_count INTEGER NOT NULL DEFAULT 0, cancel_requested INTEGER NOT NULL DEFAULT 0,
                worker_id TEXT, lease_token TEXT, lease_expires_at TEXT, last_heartbeat_at TEXT,
                next_attempt_at TEXT NOT NULL, created_at TEXT NOT NULL, started_at TEXT,
                finished_at TEXT, updated_at TEXT NOT NULL,
                FOREIGN KEY(project_id) REFERENCES projects(project_id)
            );
            CREATE TABLE IF NOT EXISTS generation_job_events (
                event_id INTEGER PRIMARY KEY AUTOINCREMENT, project_id TEXT NOT NULL, job_id TEXT NOT NULL,
                event_type TEXT NOT NULL, details_json TEXT NOT NULL DEFAULT '{}', created_at TEXT NOT NULL,
                FOREIGN KEY(job_id) REFERENCES generation_jobs(job_id)
            );
            """
        )
        _ensure_columns(cursor, "generation_job_events", {"project_id": "TEXT NOT NULL DEFAULT ''"})
        for statement in (
            ...
            "CREATE INDEX IF NOT EXISTS idx_test_points_requirement_atom ON coverage_test_points(project_id,requirement_id,atomic_requirement_id,status)",
            "CREATE INDEX IF NOT EXISTS idx_case_test_points_point ON case_test_point_links(project_id,test_point_id)",
            "CREATE INDEX IF NOT EXISTS idx_generation_jobs_claim ON generation_jobs(status,next_attempt_at,lease_expires_at,created_at)",
            "CREATE INDEX IF NOT EXISTS idx_generation_jobs_project ON generation_jobs(project_id,created_at)",
            "CREATE INDEX IF NOT EXISTS idx_generation_job_events_job ON generation_job_events(job_id,event_id)",
