...
def build_project_export_rows(
        ...
        case_rows.append(
            {
                ...
                "atomic_requirement_id": case_json.get("atomic_requirement_id", ""),
                "test_point_id": case_json.get("test_point_id", ""),
                "test_point_scenario_type": case_json.get("test_point_scenario_type", ""),
                "generation_batch_id": case_json.get("generation_batch_id", ""),
    ...
    requirement_nodes = scoped_rows("SELECT section_number,identifier,name,level,parent_id,ancestor_identifiers_json,hierarchy_path_json,node_type,sections_json,source_document,source_position_json,section_evidence_json,overview_node_id,testable,review_status,need_human_confirm FROM requirement_nodes WHERE project_id=? AND enabled=1 AND deleted_at IS NULL ORDER BY source_block_id")
    ...
    indicators = scoped_rows("SELECT i.indicator_id,i.capability_id,i.function_id,i.indicator_text,i.indicator_type,i.source_json,i.rules_json,i.verification_scope,i.need_human_confirm FROM requirement_indicators i WHERE i.project_id=? AND EXISTS(SELECT 1 FROM requirement_nodes n WHERE n.project_id=i.project_id AND n.identifier=i.function_id AND n.enabled=1 AND n.deleted_at IS NULL) ORDER BY i.function_id,i.indicator_id")
    ...
    page_links=scoped_rows("SELECT l.link_id,'page' AS binding_type,l.function_id,l.page_id,'' AS confirmed_element_id,l.confidence,l.reason,l.status,l.need_human_confirm,l.model_name,l.evidence_json FROM requirement_page_links l WHERE l.project_id=? AND EXISTS(SELECT 1 FROM requirement_nodes n WHERE n.project_id=l.project_id AND n.identifier=l.function_id AND n.enabled=1 AND n.deleted_at IS NULL) ORDER BY l.function_id")
    ...
    element_links = scoped_rows("SELECT l.link_id,'element' AS binding_type,l.indicator_id,l.page_id,l.confirmed_element_id,l.confidence,l.reason,l.status,l.need_human_confirm,l.candidates_json FROM requirement_element_links l WHERE l.project_id=? AND EXISTS(SELECT 1 FROM requirement_indicators i JOIN requirement_nodes n ON n.project_id=i.project_id AND n.identifier=i.function_id WHERE i.project_id=l.project_id AND i.indicator_id=l.indicator_id AND n.enabled=1 AND n.deleted_at IS NULL) ORDER BY l.indicator_id")
    atomic_matrix = scoped_rows("SELECT l.indicator_id,l.case_id,l.case_version,l.step_numbers_json,l.coverage_type,l.coverage_status FROM case_indicator_links l WHERE l.project_id=? AND EXISTS(SELECT 1 FROM requirement_indicators i JOIN requirement_nodes n ON n.project_id=i.project_id AND n.identifier=i.function_id WHERE i.project_id=l.project_id AND i.indicator_id=l.indicator_id AND n.enabled=1 AND n.deleted_at IS NULL) ORDER BY l.indicator_id,l.case_id")
    test_point_matrix = scoped_rows("""SELECT p.requirement_id,p.atomic_requirement_id,p.test_point_id,
        p.title,p.scenario_type,p.priority,p.evidence_requirement,p.status,l.case_id,l.generation_batch_id,
        l.data_variant,l.scenario_variant FROM coverage_test_points p LEFT JOIN case_test_point_links l
        ON l.project_id=p.project_id AND l.test_point_id=p.test_point_id
        WHERE p.project_id=? ORDER BY p.requirement_id,p.atomic_requirement_id,p.test_point_id,l.case_id""")
    ...
    return {
        ...
        "test_point_coverage_matrix": test_point_matrix,
...
def export_project_excel(manager: ProjectManager, project_id: str) -> Path:
    ...
    for key, title in (
        ...
        ("test_point_coverage_matrix", "测试点追踪矩阵"),
