...
$env:PYTHONUTF8 = "1"
$ownedStreamlit = $null
$workerSupervisor = Start-Process -FilePath "powershell.exe" -ArgumentList @("-NoProfile","-ExecutionPolicy","Bypass","-File",(Join-Path $PSScriptRoot "run-generation-worker-supervisor.ps1"),"-Python",$Python) -WorkingDirectory $projectRoot -WindowStyle Hidden -PassThru
...
try { & $Python -m desktop_pet.app *>> (Join-Path $logDir "desktop.log") }
    ...
    # The generation supervisor is intentionally not stopped here. It owns
    # durable overnight jobs independently of the UI/desktop-pet lifecycle.
}
