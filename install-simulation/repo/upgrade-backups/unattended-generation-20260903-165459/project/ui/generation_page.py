...
def _requirement_mode(service: Any, project_id: str, case_library: Any, config: dict[str, Any]) -> None:
    ...
    st.subheader("测试用例生成策略")
    count_mode = right.radio(
        "生成策略", ["按原子需求完整覆盖（推荐）", "人工限制总用例数（高级）"], horizontal=False,
        key=f"requirement_count_mode_{project_id}",
        help="系统先规划独立测试点，再按原子需求自动分批生成。",
    )
    auto_case_count = count_mode.startswith("按原子需求")
    count = right.number_input(
        "每个原子需求最大用例数", 1, 20, 10, key=f"requirement_atom_count_limit_{project_id}",
        help="这是单个原子需求的上限；实际数量由有独立价值的测试场景决定。",
    )
    manual_total_limit = None if auto_case_count else right.number_input(
        "总用例数上限（高级）", 1, 500, 100, key=f"requirement_total_count_limit_{project_id}")
    coverage_labels = {"正向路径": "normal", "异常/反向路径": "abnormal", "边界条件": "boundary",
                       "状态/约束": "state", "业务约束": "constraint", "恢复路径（适用时）": "recovery"}
    coverage_types = [value for label, value in coverage_labels.items()
                      if st.checkbox(label, value=True, key=f"coverage_{value}_{project_id}")]
    try:
        atom_count = len(service.traceability_rows(project_id).get("atomic_requirements") or [])
    except Exception:
        atom_count = 0
    if atom_count:
        st.info(f"当前项目已解析原子需求：{atom_count} 个。系统将分别规划测试点并自动分批生成，不要求模型一次输出全部用例。")
    ...
    st.info(
        ...
        + f"每个原子需求最多 {int(count)} 条；系统按测试点自动分批。"
    )
    ...
    if generate_col.button("按需求生成测试用例", type="primary", key=f"generate_{project_id}"):
        request_data = {
            ...
            "coverage_types": coverage_types, "manual_total_limit": manual_total_limit,
        }
        ...
        try:
            generation_request = GenerationRequest(
                    ...
                    generation_strategy="atomic_complete" if auto_case_count else "manual_total_limit",
                    max_cases_per_atom=int(count), manual_total_case_limit=manual_total_limit,
            ...
            job = service.enqueue_generation_job(generation_request, selected_ids, batch_id,
                                                 force=force_regenerate)
            st.session_state[f"generation_job_id_{project_id}"] = job["job_id"]
            st.success(f"后台任务已提交：{job['job_id']}。可以关闭或刷新页面，worker 会继续执行。")
        except Exception as exc:
            logger.exception("generation job submission failed project_id=%s", project_id)
            st.error(f"后台任务提交失败：{type(exc).__name__}: {exc}")
    jobs = service.generation_job_history(project_id)
    if jobs:
        st.subheader("后台生成任务")
        if st.button("刷新任务状态", key=f"refresh_generation_jobs_{project_id}"):
            st.rerun()
        for job in jobs[:5]:
            total=max(int(job.get("progress_total") or 0),1); current=int(job.get("progress_current") or 0)
            st.progress(min(current/total,1.0), text=f"{job['job_id']} · {job['status']} · {current}/{total}")
            st.caption(f"阶段：{job.get('stage') or '-'}；最近消息：{job.get('last_message') or '-'}；心跳：{job.get('last_heartbeat_at') or '-'}")
            if job.get("last_error"): st.warning(job["last_error"])
            if job["status"] in {"queued","running","retry_wait"}:
                if st.button("停止本次生成", key=f"cancel_{job['job_id']}"):
                    service.cancel_generation_job(project_id, job["job_id"]); st.rerun()
            if job["status"] in {"completed","needs_review","failed","cancelled"} and job.get("result"):
                st.session_state[f"requirement_result_{project_id}"] = job["result"]
