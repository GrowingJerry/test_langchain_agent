...
class Settings(BaseModel):
    ...
    ollama_model: str = "qwen:35b"
    ollama_extraction_model: str = "qwen:35b"
    ollama_review_model: str = "qwen:35b"
    ollama_embed_model: str = "nomic-embed-text"
    ollama_vision_model: str = "qwen2.5vl:3b"
    text_model: str = "qwen:35b"
    vision_model: str = "qwen2.5vl:3b"
    requirement_atomizer_model: str = "qwen:35b"
    requirement_auditor_model: str = "qwen:35b"
    page_understanding_model: str = "qwen2.5vl:3b"
    test_case_model: str = "qwen:35b"
    test_case_review_model: str = "qwen:35b"
    ...
    # ``generation_max_cases_per_requirement`` is retained only for old .env files.
    # New generation is bounded per atom/model call; the requirement limit is a fuse.
    generation_min_cases_per_atom: int = Field(default=4, ge=1, le=20)
    generation_default_max_cases_per_atom: int = Field(default=10, ge=1, le=20)
    generation_absolute_max_cases_per_atom: int = Field(default=20, ge=1, le=100)
    generation_max_cases_per_model_call: int = Field(default=2, ge=1, le=100)
    generation_auto_batch_enabled: bool = True
    generation_coverage_repair_enabled: bool = True
    generation_coverage_repair_max_rounds: int = Field(default=2, ge=0, le=10)
    generation_hard_max_cases_per_requirement: int = Field(default=500, ge=20, le=5000)
    ...
    # Legacy single-stream wall-clock limit.  It is accepted for old .env files
    # and mapped to model_call_max_seconds only when the new key is absent.
    generation_max_seconds: int = Field(default=600, ge=10, le=86400)
    generation_model_call_max_seconds: int = Field(default=7200, ge=0, le=86400)
    generation_idle_timeout_seconds: int = Field(default=900, ge=5, le=86400)
    generation_batch_max_seconds: int = Field(default=43200, ge=0, le=604800)
    generation_job_lease_seconds: int = Field(default=300, ge=30, le=3600)
    generation_job_max_attempts: int = Field(default=20, ge=1, le=100)
    generation_job_retry_delay_seconds: int = Field(default=30, ge=1, le=3600)
    generation_worker_idle_exit_seconds: int = Field(default=120, ge=10, le=86400)
    ...
    def from_env(
        ...
        source: Dict[str, Any] = {}
        # Upgrade tooling snapshots the old installation's effective Settings
        # here before replacing this module. This preserves site-specific model,
        # URL, context and path choices without preserving obsolete safety knobs.
        preserved_path = PROJECT_ROOT / "config" / "preserved_runtime_settings.json"
        if preserved_path.is_file():
            try:
                preserved = json.loads(preserved_path.read_text(encoding="utf-8"))
                managed = {"generation_max_seconds", "generation_model_call_max_seconds",
                    "generation_idle_timeout_seconds", "generation_batch_max_seconds",
                    "generation_job_lease_seconds", "generation_job_max_attempts",
                ...
                source.update({str(key).upper(): value for key, value in preserved.items()
        ...
        if ("GENERATION_MODEL_CALL_MAX_SECONDS" not in source
                and source.get("GENERATION_MAX_SECONDS")):
            field_values["generation_model_call_max_seconds"] = source["GENERATION_MAX_SECONDS"]
        return cls.model_validate(field_values)

    @property
    def effective_model_call_max_seconds(self) -> int:
        """New setting wins; direct construction keeps the legacy value useful."""
        return self.generation_model_call_max_seconds or self.generation_max_seconds
