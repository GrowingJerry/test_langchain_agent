...
def parse_requirement_file(path: Path, test_object: str = "") -> List[RequirementItem]:
    ...
    suffix = path.suffix.lower()
    if suffix == ".txt":
        from infrastructure.documents.requirement_parser import parse_txt_file

        return parse_txt_file(path, test_object=test_object)
    if suffix in (".xlsx", ".xls"):
        from infrastructure.documents.requirement_parser import parse_excel_requirements
