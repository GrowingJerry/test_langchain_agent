...
def test_atomic_coverage_repair_calls_only_missing_test_point(workspace) -> None:
    manager, project_id, chunk_id = workspace
    atom_id = "ATOM-REPAIR"
    with manager.connections.transaction() as conn:
        conn.execute("""INSERT INTO requirement_indicators(
            project_id,indicator_id,capability_id,function_id,parent_indicator_id,indicator_text,
            indicator_type,source_json,rules_json,verification_scope,need_human_confirm
            ) VALUES(?,?,?,?,?,?,?,?,?,?,?)""",
            (project_id, atom_id, "CAP", "REQ-1", "", "订单处理", "功能", "{}", "{}", "offline", 0))
        for index in range(1, 4):
            conn.execute("""INSERT INTO coverage_test_points(
                project_id,requirement_id,atomic_requirement_id,test_point_id,title,scenario_type,
                description,priority,evidence_requirement,status,metadata_json
                ) VALUES(?,?,?,?,?,?,?,?,?,'pending','{}')""",
                (project_id, "REQ-1", atom_id, f"TP-{index}", f"测试点{index}", "normal",
                 f"验证订单场景{index}", "P1", "requirement"))
    requested_batches: list[list[str]] = []

    def generator(packages, current_request, reason):
        ids = [x["test_point_id"] for x in current_request.target_test_points]
        requested_batches.append(ids)
        selected = ids[:-1] if len(requested_batches) == 1 else ids
        return [canonical_case(chunk_id, case_id=f"TMP-{point_id}").model_copy(update={
            "requirement_ids": ["REQ-1"], "scenario_ids": [], "test_point_id": point_id,
        }) for point_id in selected]

    service = GenerationService(manager, settings=enabled_settings(
        ollama_num_ctx=65536, ollama_structured_num_predict=8192,
        generation_max_cases_per_model_call=8, generation_coverage_repair_max_rounds=2,
    ), health_client=Health(), direct_model_generator=generator)
    result = service.generate_atomic_coverage(GenerationRequest(
        project_id=project_id, requirement_ids=["REQ-1"], case_type="功能测试",
        case_count=3, max_cases_per_atom=6, requested_mode="auto", use_history=False,
    ))
    assert requested_batches == [["TP-1", "TP-2", "TP-3"], ["TP-3"]]
    assert result.coverage_status == "completed"
    assert not result.missing_test_point_ids
