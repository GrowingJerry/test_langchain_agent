...
def bind_project(manager,project_id:str,settings:Settings)->dict[str,Any]:
    ...
    return {"bindings":results,"participating":len(nodes),"confirmed":confirmed,"need_human_confirm":pending+failed,"unmatched":sum(x["status"]=="page_not_found" for x in results),"failed":failed,"model":settings.text_model}
