...
def bind_project(manager,project_id:str,settings:Settings)->dict[str,Any]:
    with manager.connections.connection() as conn:
        ...
        elements=[dict(x) for x in conn.execute("SELECT * FROM html_elements WHERE project_id=?",(project_id,))]
        observations=[dict(x) for x in conn.execute("SELECT * FROM html_observations WHERE project_id=?",(project_id,))]
    by_page={p["page_id"]:[] for p in pages}
    for element in elements: by_page.setdefault(element["page_id"],[]).append(loads_json(element["element_json"],{}))
    ...
    with manager.connections.transaction() as conn:
        for node in nodes:
            ...
            candidates=[]
            for page in pages:
                controls=by_page.get(page["page_id"],[]); page_text=" ".join([page.get("title") or "",page.get("page_path") or "",json.dumps(controls,ensure_ascii=False)])
                hits=sorted(req_tokens&_tokens(page_text)); score=len(hits)/max(1,len(req_tokens)); candidates.append({"page_id":page["page_id"],"title":page.get("title"),"path":page.get("page_path"),"score":score,"hits":hits,"controls":controls[:80]})
            candidates.sort(key=lambda x:x["score"],reverse=True); compact=[{k:v for k,v in x.items() if k!="controls"} for x in candidates[:5]]
            schema={"type":"object","required":["page_id","confidence","reason","element_ids","need_human_confirm"],"properties":{"page_id":{"type":"string"},"confidence":{"type":"number"},"reason":{"type":"string"},"element_ids":{"type":"array","items":{"type":"string"}},"need_human_confirm":{"type":"boolean"}}}
            ...
            response=requests.post(settings.ollama_base_url.rstrip("/")+"/api/chat",json={"model":settings.text_model,"stream":False,"think":False,"format":schema,"messages":[{"role":"user","content":"依据需求语义、简化DOM和真实观测选择页面及元素。关键词仅是候选，不得编造ID。只输出JSON。\n"+json.dumps(payload,ensure_ascii=False)}],"options":{"temperature":0,"num_predict":1000}},timeout=settings.ollama_timeout); response.raise_for_status(); decision=json.loads(response.json()["message"]["content"])
