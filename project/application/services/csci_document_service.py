...
def parse_formal_csci_docx(path:Path)->dict[str,Any]:
    document=Document(path); nodes=[]; overview=[]; detail=[]; stack=[]; active_part=""; active_section=""; block_no=0
    ...
    for block in _blocks(document):
        ...
        if isinstance(block,Paragraph):
            ...
            if heading:
                ...
                if number=="3.2": active_part="3.2"; active_section=""; stack=[]; current=None; continue
                if number=="3.3": active_part="3.3"; active_section=""; stack=[]; current=None; continue
                if active_part and number and not number.startswith(active_part):
                    if len(number.split("."))<=2: active_part=""; active_section=""; stack=[]; current=None
                    continue
                if not active_part: continue
                if active_part=="3.3" and title in FOUR_SECTIONS:
            ...
            if rows:
                key=active_section or ("总体需求概述" if active_part=="3.2" else "正文"); header=rows[0]
    ...
    return {"nodes":nodes,"overview_nodes":overview,"detail_nodes":detail,"testable_nodes":[x for x in complete if x.testable]}
