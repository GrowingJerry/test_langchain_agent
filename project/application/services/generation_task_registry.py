...
_lock=threading.RLock(); _events:dict[str,threading.Event]={}; _futures:dict[str,Future]={}

def _now()->str: return datetime.now(timezone.utc).isoformat()
def _atomic(path:Path,value:Any)->None:
    temp=path.with_suffix(path.suffix+".tmp"); temp.write_text(json.dumps(value,ensure_ascii=False,default=str,indent=2),"utf-8"); temp.replace(path)
...
class GenerationTaskStore:
    ...
    def result(self,task_id:str)->dict[str,Any]:
        path=self.task_dir(task_id)/"result.json"; return json.loads(path.read_text("utf-8")) if path.exists() else {}
...
def submit_generation(store:GenerationTaskStore,fn:Callable[[Callable[[],bool],Callable[...,dict[str,Any]]],Any],*,project_id:str,requirement_id:str,model:str,mode:str,total:int)->str:
    ...
    def work():
        store.update(task_id,status="running",started_at=_now()); store.emit(task_id,"status","正在构建GenerationPackage")
        try:
            result=fn(cancel.is_set,lambda kind,content,**extra:store.emit(task_id,kind,content,**extra))
            ...
            _atomic(directory/"result.json",result); reason="client_cancelled" if cancelled else (str(failed[0].get("error")) if failed else "")
            store.update(task_id,status=status,ended_at=_now(),actual_mode=actual,failure_reason=reason,termination_reason=reason if cancelled else "",generated_case_count=generated,persisted_case_count=generated,result_reference=str(directory/"result.json"),log_path=log_path,agent_to_direct=agent_to_direct,completed_requirements=len(result.get("completed") or [])+len(result.get("skipped") or []))
            store.emit(task_id,"done","生成已停止" if cancelled else ("生成失败" if status=="failed" else "生成完成"),status=status)
            return result
            ...
            status="cancelled" if cancel.is_set() else "failed"; store.update(task_id,status=status,ended_at=_now(),failure_reason=f"{type(exc).__name__}: {exc}",termination_reason=termination)
            store.emit(task_id,"error",f"{type(exc).__name__}: {exc}",termination_reason=termination); store.emit(task_id,"done","生成已停止" if status=="cancelled" else "生成失败",status=status); raise
        finally:
...
def request_cancel(store:GenerationTaskStore,task_id:str)->bool:
    ...
    store.update(task_id,status="cancel_requested",termination_reason="client_cancelled"); store.emit(task_id,"status","正在停止本次生成")
    event=_events.get(task_id)
    if event: event.set(); return True
    store.update(task_id,status="cancelled",ended_at=_now()); store.emit(task_id,"done","生成已停止",status="cancelled"); return True
