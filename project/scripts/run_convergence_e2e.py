...
def one_flow(manager,service,name,doc,expected_method):
    ...
    binding=run_step(records,'语义绑定',service.binding_funnel(project_id),lambda:service.auto_bind_requirements(project_id),lambda x:x['participating']); assert binding['participating']==binding['confirmed']+binding['need_human_confirm']
...
def main():
    ...
    report={'database':str(db),'models':['qwen3:8b','qwen2.5vl:3b'],'flows':flows,'restart_persisted_cases':persisted}; path=root/'acceptance-report.json'; path.write_text(json.dumps(report,ensure_ascii=False,indent=2),'utf-8'); print(json.dumps(report,ensure_ascii=False,indent=2))
