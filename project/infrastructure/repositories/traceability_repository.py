...
class TraceabilityRepository(BaseRepository):
    ...
    def accept_version(self, project_id: str, case_id: str, version_no: int) -> dict[str, Any]:
        versions=self.list_case_versions(project_id,case_id); selected=next((x for x in versions if x["version_no"]==version_no),None)
        ...
        with self.connections.transaction() as conn:
            ...
            conn.execute("UPDATE case_versions SET acceptance_status='accepted' WHERE project_id=? AND case_id=? AND version_no=?",(project_id,case_id,version_no))
            conn.execute("UPDATE generated_cases SET case_json=? WHERE project_id=? AND case_id=?",(json.dumps(selected["case_json"],ensure_ascii=False),project_id,case_id))
        return selected
